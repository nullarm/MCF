	...
	!----------------------------------------------------
        ...
        iline = 0
          ...
          ELSE IF (carg == 'CHI2') THEN
             ...
             CALL physics_set_chi2(phys,chi2,stat_info_sub)       
          ...
          ELSE IF(carg == 'NUM_COLLOID' ) THEN
             ...
             cc_repul_F0      = 0.0_MK
             cc_magnet_cut_off = 0.0_MK
             cc_magnet_cut_on  = 0.0_MK
             cc_magnet_F0      = 0.0_MK
             cc_magnet_B(:)    = 0.0_MK
             cc_magnet_mom(:)  = 0.0_MK
             cc_magnet_f       = 0.0
             cc_magnet_chi     = 0.0
             cc_magnet_mu      = 1.0
          ...
          ELSE IF (carg =='COLL_IMPLICIT_PAIR_SWEEP_ADAPTIVE' .AND. &
             ...
             !-----------------------------------------------
             ! colloids implicit sweeps is adaptive
             !-----------------------------------------------
             
             READ(cvalue,'(L)',IOSTAT=ios,ERR=200) coll_implicit_pair_sweep_adaptive
             
          ELSE IF ( carg == 'COLL_IMPLICIT_PAIR_SWEEP_TOLERANCE'  .AND. &
               num_species > 1 .AND. &
               num_colloid > 0 ) THEN
             
             !-----------------------------------------------
             ! colloids implicit tolerance of sweeps
             !-----------------------------------------------
          ...
          ELSE IF(carg == 'CC_REPUL_F0' .AND. &
             ...
             !-----------------------------------------------
             !  colloid-colloid magnetic force cut off.
             !-----------------------------------------------
             
          ELSE IF(carg == 'CC_MAGNET_CUT_OFF' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_cut_off
             
             !-----------------------------------------------
             !  colloid-colloid magnetic force cut on
             !-----------------------------------------------
             
          ELSE IF(carg == 'CC_MAGNET_CUT_ON' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_cut_on
             
             !-----------------------------------------------
             !  colloid-colloid magnetic force magnitude.
             !-----------------------------------------------
             
          ELSE IF(carg == 'CC_MAGNET_F0' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_F0
          ...
          ELSE IF (carg == 'CC_MAGNET_B' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0 ) THEN
             
             READ(cvalue,*,IOSTAT=ios, ERR=200)  &
                  cc_magnet_B(1:num_dim)
          ...
          ELSE IF (carg == 'CC_MAGNET_MOM' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0 ) THEN
          ...
          ELSE IF(carg == 'CC_MAGNET_F' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_f

             !-----------------------------------------------
             !  magnetic susceptibility difference between
             !  a colloid and fluid.
             !-----------------------------------------------
             
          ELSE IF(carg == 'CC_MAGNET_CHI' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_chi
          ...
          ELSE IF(carg == 'CC_MAGNET_MU' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_mu
