...
#if   __KIND == __SINGLE_PRECISION | __KIND == __DOUBLE_PRECISION
      REAL(MK)   , DIMENSION(:)       , POINTER :: abuffer=>null(),bbuffer=>null()
#elif __KIND == __SINGLE_PRECISION_COMPLEX | __KIND == __DOUBLE_PRECISION_COMPLEX
      COMPLEX(MK), DIMENSION(:)       , POINTER :: abuffer=>null(),bbuffer=>null()
#elif __KIND == __INTEGER
      INTEGER    , DIMENSION(:)       , POINTER :: abuffer=>null(),bbuffer=>null()
#elif __KIND == __LOGICAL
      LOGICAL    , DIMENSION(:)       , POINTER :: abuffer=>null(),bbuffer=>null()
#endif
#endif
      !-------------------------------------------------------------------------
      !  Externals 
 ...
 600          CALL MPI_Allreduce(info,info2,1,MPI_INTEGER,MPI_SUM,ppm_comm, &
     ...
     &                        lchar,info)
                          ...
                          IF (ibuffer+nrec .LE. ndata) THEN
                              ...
                              bbuffer(ibuffer+1:ibuffer+nrec) = rbuf
                              ibuffer = ibuffer + nrec
                              jbuffer = nrec
                          ELSE
                              ! abuffer too small. keep remainder
                              jbuffer = ndata - ibuffer
                              bbuffer(ibuffer+1:ndata) = rbuf(1:jbuffer)
                              ibuffer = ndata
                          ENDIF
                      ENDDO      ! WHILE ibuffer.LT.ndata
                  ELSEIF (ifmt .EQ. ppm_param_io_ascii) THEN
                      ...
                      IF (ifmt .EQ. ppm_param_io_binary) THEN
                          ...
                          IF (jbuffer .LT. nrec) THEN
                              ...
                              IF (rem .LE. ndata) THEN
                                  ...
                                  abuffer(1:rem) = rbuf(jbuffer+1:nrec)
                                  ibuffer = rem
                                  jbuffer = nrec
                              ELSE
                                  ! abuffer too small. keep remainder
                                  abuffer(1:ndata) =    &
     &                                rbuf(jbuffer+1:jbuffer+ndata)
     ...
     &                            lchar,info)
                              ...
                              IF (ibuffer+nrec .LE. ndata) THEN
                                  ! abuffer can hold whole record
                                  abuffer(ibuffer+1:ibuffer+nrec) = rbuf
                                  ibuffer = ibuffer + nrec
                                  jbuffer = nrec
                              ELSE
                                  ! abuffer too small. keep remainder
                                  jbuffer = ndata - ibuffer
                                  abuffer(ibuffer+1:ndata) = rbuf(1:jbuffer)
                                  ibuffer = ndata
                              ENDIF
                          ENDDO      ! WHILE ibuffer.LT.ndata
                      ELSEIF (ifmt .EQ. ppm_param_io_ascii) THEN
