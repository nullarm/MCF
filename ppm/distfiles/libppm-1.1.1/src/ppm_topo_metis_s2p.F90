...
#include "ppm_define.h"
      ...
      !  Local variables 
      !-------------------------------------------------------------------------
      INTEGER , DIMENSION(:  ), POINTER   :: elmnts=>null(),npart=>null(),nxadj=>null(),nadjncy=>null()
      INTEGER , DIMENSION(:  ), POINTER   :: vwgt=>null(),adjwgt=>null(),vsize=>null(),vote=>null()
  ...
  !    NULLIFY(vsize)    ! we make no use of communication weights
     ...
     &           'METIS partition weights TPWGT',__LINE__,info)
      ...
      IF (assig .EQ. ppm_param_assign_nodal_cut .OR.   &
     &    assig .EQ. ppm_param_assign_nodal_comm) THEN
         print *, __FILE__, ': ', __LINE__, 'Calling Metis'
