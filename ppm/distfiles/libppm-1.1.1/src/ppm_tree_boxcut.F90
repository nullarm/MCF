...
#include "ppm_define.h"
     ...
     &            'list of particle box IDs PBOX',__LINE__,info)
          ...
          !---------------------------------------------------------------------
#ifndef __BOXCUTLOGICALS
          IF (ncut .EQ. 1) THEN
              DO j=1,Np
                  ip = tree_lpdx(tree_lhbx(1,cutbox)+j-1)
                  IF (xp(cutdir(1),ip).LT.cutpos(1)) THEN
                     pbox(j) = 1
                  ELSE
                     pbox(j) = 2
                  END IF
          ...
          ELSEIF (ncut .EQ. 2) THEN
              DO j=1,Np
                  ...
                  ELSE
                     pbox(j) = 2
                  END IF
                  IF (xp(cutdir(2),ip).LT.cutpos(2)) THEN
                     pbox(j) = pbox(j) + 1
                  ELSE
                     pbox(j) = pbox(j) + 2
                  END IF
          ...
          ELSEIF (ncut .EQ. 3) THEN
              DO j=1,Np
                  ip = tree_lpdx(tree_lhbx(1,cutbox)+j-1)
                  IF (xp(cutdir(3),ip).LT.cutpos(3)) THEN
                     pbox(j) = 0
                  ELSE
                     pbox(j) = 4
                  END IF
          ...
          IF (ncut .EQ. 1) THEN
              DO j=1,Np
                  ip = tree_lpdx(tree_lhbx(1,cutbox)+j-1)
                  pbox(j) = LOGICAL(.NOT.(xp(cutdir(1),ip).LT.cutpos(1)),   &
     &                      KIND(1)) + 1
              ENDDO
          ELSEIF (ncut .EQ. 2) THEN
              DO j=1,Np
                  ip = tree_lpdx(tree_lhbx(1,cutbox)+j-1)
                  pbox(j) = 2*LOGICAL(.NOT.(xp(cutdir(1),ip).LT.cutpos(1)), &
     &                      KIND(1)) + LOGICAL(.NOT.(xp(cutdir(2),ip)       &
     &                      .LT.cutpos(2)),KIND(1)) + 1
              ENDDO
          ELSEIF (ncut .EQ. 3) THEN
              DO j=1,Np
                  ip = tree_lpdx(tree_lhbx(1,cutbox)+j-1)
                  pbox(j) = 4*LOGICAL(.NOT.(xp(cutdir(3),ip).LT.cutpos(3)), &
     &                      KIND(1)) + 2*LOGICAL(.NOT.(xp(cutdir(1),ip)     &
     &                      .LT.cutpos(1)),KIND(1)) + LOGICAL(.NOT.         &
     &                      (xp(cutdir(2),ip) .LT. cutpos(2)),KIND(1)) + 1
              ENDDO
          ENDIF
#endif
          !---------------------------------------------------------------------
